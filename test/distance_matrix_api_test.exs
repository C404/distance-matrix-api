  ...
  test ".distances response for success" do
    use_cassette "success" do
      travels = DistanceMatrixApi.TravelList.new |>
        ...
        DistanceMatrixApi.TravelList.add_entry(%{origin: %{lat: 45.764043, long: 4.835658999999964},
          destination: %{lat: 48.856614, long: 2.3522219000000177}})

      response = travels |> DistanceMatrixApi.distances

      assert response != %{}
      assert response["origin_addresses"] == ["Caen, France", "Lyon, France",
        "21 Rue de la République, 69002 Lyon, France"]
      assert response["destination_addresses"] == ["Paris, France", "Nice, France",
